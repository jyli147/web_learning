...
import {
  ...
  UpdateTasksClearCompletedViewEvent,
} from "./tasks_clear_completed_view.js";
import { TasksCounterStore } from "./tasks_counter_store.js";
import {
  ButtonAddTasksView,
  UpdateButtonAddTasksViewEvent,
} from "./button_add_tasks_view.js";
import {
  ButtonAddCategoryView,
  UpdateButtonAddCategoryViewEvent,
} from "./button_add_category_view.js";
import {
  ModalTasksView,
  UpdateCloseModalTasksViewEvent,
  AddTaskRequestModalViewEvent,
} from "./modal_task_view.js";
import {
  ModalCategoriesView,
  UpdateCloseModalCategoriesViewEvent,
  AddCategoryRequestModalViewEvent,
  UpdateColorCategoriesViewEvent,
} from "./modal_categories_view.js";
...
class App {
  ...
  init() {
    ...
    this.#tasksView.render(
      this.#tasksStore.findAll(),
      this.#filtersStore.activeFilter?.current,
    );
    this.#filtersView.render(this.#filtersStore.activeFilter);
    this.#categoriesView.render(this.#categoriesStore.findAll());
    this.#tasksCounterView.render(
      this.#tasksCounterStore.findAmountOfTasksByFilter(
        this.#tasksStore.findAll(),
        this.#filtersStore.activeFilter.current,
      ),
    );
    ...
    this.#categoriesStore.addEventListener(
      UpdateCategoriesStoreEvent.type,
      (e) => {
    ...
    this.#tasksView.addEventListener(
      UpdateTaskIsCompletedViewEvent.type,
      (e) => {
        this.#tasksStore.toggleTaskById(e.id);
      },
    );

    this.#tasksClearCompletedView.addEventListener(
      UpdateTasksClearCompletedViewEvent.type,
      (e) => {
        this.#tasksStore.deleteAll();
      },
    );

    this.#buttonAddTasksView.addEventListener(
      UpdateButtonAddTasksViewEvent.type,
      (e) => {
        this.#modalTasksView.openModal(this.#categoriesStore.findAll());
      },
    );

    this.#buttonAddCategoryView.addEventListener(
      UpdateButtonAddCategoryViewEvent.type,
      (e) => {
        this.#modalCategoriesView.openModal();
      },
    );

    this.#modalTasksView.addEventListener(
      UpdateCloseModalTasksViewEvent.type,
      (e) => {
        this.#modalTasksView.closeModal(e);
      },
    );

    this.#modalCategoriesView.addEventListener(
      UpdateCloseModalCategoriesViewEvent.type,
      (e) => {
    ...
    this.#modalTasksView.addEventListener(
      AddTaskRequestModalViewEvent.type,
      (e) => {
    ...
    this.#modalCategoriesView.addEventListener(
      AddCategoryRequestModalViewEvent.type,
      (e) => {
